...
La partición de los datos es estrictamente temporal, sin aleatorización, de forma coherente con el carácter causal de las imputaciones y para evitar cualquier fuga de información del futuro. El conjunto de entrenamiento abarca de 2019 a 2024; el conjunto de validación comprende los meses de enero a septiembre de 2025 y se emplea para la selección de órdenes e hiperparámetros con control de sobreajuste; y el conjunto de test corresponde a los meses de octubre y noviembre de 2025, el periodo más reciente disponible, que permanece intacto durante todo el entrenamiento y la selección. La modelización se realiza por código postal, tratando cada uno como una serie temporal independiente.

\subsubsection{Backtesting con ventana deslizante}

La capacidad predictiva se estima mediante backtesting con ventana deslizante (\textit{walk-forward}): el modelo se ajusta sobre el conjunto de entrenamiento, genera una predicción para el horizonte fijado, avanza la ventana incorporando las observaciones reales y repite el proceso a lo largo del periodo de evaluación. Se consideran horizontes de 24, 48 y 72 horas, siendo el de 72 horas el principal por su mayor margen para la alerta temprana. El mismo arnés de backtesting se aplica de forma idéntica a todos los modelos, con un modelo \textit{naive} estacional (consumo del mismo bloque de la semana anterior) como referencia mínima.
