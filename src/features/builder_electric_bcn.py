...
class DatasetBuilder(BaseETL):
    ...
    def extract(self):
        ...
        self.datos_electricity = pl.DataFrame(list(self.db["clean_electricity"].find({}, {"_id": 0})))
    ...
    def _build_mapeo_estaciones(self) -> pl.DataFrame:
        ...
        def estacion_mas_cercana(lon, lat):
            return min(self.ESTACIONES, key=lambda e:
                (self.ESTACIONES[e][0] - lon) ** 2 + (self.ESTACIONES[e][1] - lat) ** 2
            )
    ...
    def _build_dataset(self, electricity, modis, festivos, meteocat) -> pl.DataFrame:
        ...
        dataset = (
            electricity
            .join(modis, how="left", on=["cod_postal", "datetime"])
