...
class OpenDataBcnIngester(BaseETL):
    ...
    def extract(self) -> pl.DataFrame:
        ...
        for res in resources:
            self.logger.info(f" Catalogo disponible: {res['name']} | id: {res['id']} | url: {res.get('url', 'sin url')}")
        ...
        for res in resources:
            ...
            for attempt in range(3):
                try:
                    ...
                    self.logger.error(f"Error al descargar recurso {res['name']} en intento {attempt + 1}: {e}")
                    time.sleep(5) # Espera antes de reintentar
                    continue
            else:
                self.logger.error(f"No se pudo descargar el recurso {res['name']} después de 3 intentos")
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        raw_od_bcn_lst = list(self.raw_collection.find({}, {"_id": 0})) #Lee los datos raw de MongoDB, excluyendo el campo _id
        ...
        raw_od_bcn = raw_od_bcn.filter(pl.col("Tram_Horari") != "No consta") #Elimina registros con "No consta" en Tram_Horari
        ...
        raw_od_bcn = (
            ...
            ).drop("Any","Data","Tram_Horari","Valor").select("Datetime", "Codi_Postal", "Sector_Economic", "MWh")
    ...
    def load_clean(self, df: pl.DataFrame) -> None:
        ...
        self.clean_collection.insert_many(df.to_dicts()) #Convierte el DataFrame a una lista de diccionarios y los inserta en MongoDB
