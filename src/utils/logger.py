...
def get_logger(name:str) -> logging.Logger:
    ...
    logger.setLevel(logging.INFO) #Establece el nivel de logueo a INFO, lo que significa que se registrarán mensajes de nivel INFO y superiores (WARNING, ERROR, CRITICAL)

#Handler define a donde van los mensajes
    if not logger.hasHandlers(): #Verifica si el logger ya tiene handlers configurados para evitar agregar múltiples handlers al mismo logger
        handler = logging.StreamHandler(sys.stdout) #Crea un handler que envía los mensajes de logueo a la salida estándar (consola)
