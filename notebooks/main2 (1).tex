...
Los modelos baseline establecen la referencia de rendimiento contra la que se comparan las familias de mayor complejidad. Se construyen tres modelos de complejidad creciente. Comenzando con un \textit{naive} estacional, un SARIMA y un finalmente un SARIMAX. Estos modelos clásicos son ajustados de forma independiente por código postal y evaluados bajo la misma estructura de backtesting con ventana deslizante descrito en la metodología.
...
La estructura de los modelos SARIMA se deriva del análisis de la serie realizado en el EDA. Las pruebas de estacionariedad mostraron  una estacionariedad parcialya que el test ADF rechaza la hipótesis de raíz unitaria, confirmando estacionariedad en media, mientras que el KPSS la rechaza en varianza, cuya componente no estacionaria es de naturaleza estacional. 

En consecuencia se fija una diferenciación regular nula ($d=0$), para no sobre-diferenciar una serie sin tendencia sostenida, y una diferenciación estacional unitaria ($D=1$) con periodo $s=28$ que absorbe el ciclo semanal. Las funciones de autocorrelación simple y parcial (ACF/PACF) orientan los órdenes autorregresivo y de media móvil.  PACF mantiene contribuciones directas significativas hasta el tercer o cuarto retardo y la ACF decae de forma gradual, lo que motivó  un orden de partida $(p,d,q)=(3,0,1)$ y $(P,D,Q)_s=(1,1,1)_{28}$, en línea con la metodología clásica.
...
Por lo mencionado anterioremnte, se aplica el criterio de respaldo, seleccionar la combinación de mayor R\textsuperscript{2} de validación documentando su diferencia relativa. La búsqueda se realiza sobre cuatro códigos postales representativos de los perfiles del EDA (industrial 08038, mixto 08032, de servicios 08002 y residencial 08027) y el orden ganador se aplica de forma común a los 30 códigos postales del conjunto de evaluación, para garantizar una comparación homogénea y reducir el coste computacional. El orden resultante es $(2,0,1)(1,1,1)_{28}$, que alcanza un R\textsuperscript{2} de validación prácticamente equivalente al de orden tres de 0,605 frente a 0,610 de media sobre los cuatro códigos. Debido a su mayor sencillez, se elige como la mejor combinación.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{SarimaComb.png}
    \caption{Selección de Orden Sarima, R2 overfitting}
    \label{fig:placeholder}
\end{figure}

\subsubsection{Variables exógenas del SARIMAX}

El SARIMAX amplía el SARIMA incorporando variables externas conocidos en el momento de la predicción, agrupados en tres bloques. El bloque climático incluye los grados-día de calefacción y refrigeración (\texttt{HDD}, \texttt{CDD}) y la humedad media; la temperatura máxima y mínima se excluye deliberadamente por su elevada colinealidad con los grados-día (correlación de Spearman entre 0,81 y 0,86), que en un modelo lineal desestabiliza la estimación de los coeficientes. El bloque de calendario aporta los indicadores de festivo (\texttt{es\_festivo}) y de pandemia (\texttt{is\_covid}). Los rezagos del consumo no se incorporan como variables exógenas, puesto que la dependencia autorregresiva ya la modela el propio componente AR del SARIMA.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{BayesianLgmb.png}
    \caption{Optimizacion GridSearch vs Bayesiana}
    \label{fig:placeholder}
\end{figure}
El control de sobreajuste requiere una adaptación para los modelos de árboles. Su $R^2$ in-sample es próximo a uno por construcción y no resulta informativo. En su lugar, la diferencia relativa se calcula entre dos evaluaciones out-of-sample al mismo horizonte de 72 horas y sobre ventanas simétricas. El modelo entrenado hasta 2023 y evaluado sobre enero--septiembre de 2024 (rendimiento en la era de entrenamiento) frente al modelo entrenado hasta 2024 y evaluado sobre la validación de enero--septiembre de 2025.

Igual que en el SARIMA, ningún candidato baja del umbral del 10\%. El criterio es seleccionar el de mayor $R^2$ de validación documentando su diferencia relativa. Finalmente, el escalado individual de cada serie  mejora tanto la precisión, especialmente en los barrios de menor consumo, que el modelo global, como la generalización, reduciendo la diferencia relativa. El modelo final alcanza un $R^2$ mediano de validación de 0,689 con una diferencia relativa del 12,7\%, frente al 0,627 del modelo con hiperparámetros por defecto.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{OverfitLgbm.png}
    \caption{Overfitting LightGBM}
    \label{fig:placeholder}
\end{figure}

Sobre el conjunto de test (octubre--noviembre de 2025), el modelo final alcanza un
$R^2$ mediano de 0,837 y un MAPE mediano del 7,1\% (WMAPE 6,9\%) a horizonte de 72
horas. La figura desglosa este resultado por código postal. La mayoría se sitúa por encima de la mediana, con valores superiores a 0,90 en zonas céntricas y residenciales de consumo estable como el 08001 (El Raval), el 08012 (Gràcia) o el 08002 (Barri Gòtic).

En el extremo opuesto, un grupo reducido de códigos postales periféricos e
industriales queda muy por debajo: el 08039 (Port de Barcelona) con un $R^2$ de 0,58,
el 08040 (Zona Franca) con un $R^2$ de 0,44 y, sobre todo, el 08033 (Ciutat
Meridiana--Torre Baró) con un $R^2$ de 0,23. Son series con un consumo errático y sin
un ciclo diario estable que el modelo pueda aprender; como se detalla en el
capítulo~V, esta es una carácterística del barrio y no se explica por su volumen ni
por su perfil sectorial.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{MapaBCN-R2-lIGHTgbm.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{MuestralLightGBM.png}
    \caption{LightGMB predicción muestral vs extramuestral}
    \label{fig:placeholder}
\end{figure}
El análisis de residuos muestra errores centrados en cero (ausencia de sesgo sistemático) con heterocedasticidad creciente con la magnitud del consumo, comportamiento característico de la demanda eléctrica que refuerza el uso de métricas relativas. La comparativa sistemática frente a los baselines se presenta en el capítulo~V.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{ErrorLightGBM.png}
    \caption{LightGBM R2 por código postal}
    \label{fig:placeholder}
\end{figure}
 
 Tal como se esperaba, El R2 final se degrada en cuanto más se alarga el horizonte de predicción. 72 horas 0,878 a 24 horas, 0,851 a 48 y 0,837 a 72.

 \begin{figure}
     \centering
     \includegraphics[width=0.5\linewidth]{HorizLightGBM.png}
     \caption{Degradación de R2 con por horizonte}
     \label{fig:placeholder}
 \end{figure}
...
El XGBoost se evalúa con la misma estructura global multi-serie que el LightGBM, cambiando únicamente el regresor.. La única
...
La optimización emplea una búsqueda bayesiana de 30 iteraciones, igual que el LightGBM pero descartando Grid Search, sobre los hiperparámetros propios de XGBoost: profundidad, tasa de aprendizaje, submuestreo de filas y columnas, y la regularización mediante \texttt{gamma}, \texttt{min\_child\_weight} y los términos L1/L2. El candidato elegido eleva el $R^2$ de validación a 0,693 y reduce la diferencia relativa a 0,132.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{BusquedaXGBoost.png}
    \caption{Overfitting XGBoost}
    \label{fig:placeholder}
\end{figure}

Dado que la diferencia relativa no bajó tanto, se realiza una segunda ronda de regularización probando cinco cambios diferetnes sobre el candidato: escalado de las series, transformación logarítmica del objetivo, reducción del peso del periodo COVID, crecimiento del árbol por hoja (\texttt{lossguide}, que emula el comportamiento de LightGBM) y poda del número de árboles. Ninguna baja la diferencia relativa y el escalado, la única que mejora la precisión, solo aporta 0,01 puntos de $R^2$. Aplicando la regla de un error estándar~\cite{hastie2009} (el error estándar del $R^2$ entre los 30 códigos es de unos 0,022, muy superior a esa mejora de 0,01), las dos configuraciones son estadísticamente indistinguibles, por lo que se elige el modelo más sencillo, sin escalado (figura~\ref{fig:seleccion_xgb}). La decisión es además coherente
con la naturaleza de los árboles, invariantes a las transformaciones monótonas de los predictores, a diferencia de lo observado en el LightGBM, donde el escalado sí aportó una mejora consistente.

Sobre el conjunto de test, el XGBoost final alcanza un $R^2$ mediano de 0,846 (media 0,800) y un WMAPE mediano del 6,3\,\% (MAPE 6,6\,\%) a 72 horas, mejorando al modelo por defecto (que en test rinde 0,829) en 27 de los 30 códigos postales. Es, nominalmente, el mejor modelo del estudio, si bien su ventaja sobre el LightGBM es muy estrecha, de apenas 0,009 puntos de $R^2$ mediano (sección~V.A). 

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{MapXGBoost.png}
    \caption{R2 Map XGBoost}
    \label{fig:placeholder}
\end{figure}

El análisis de residuos reproduce el patrón del LightGBM (errores centrados en cero con heterocedasticidad creciente; véase figura~\ref{fig:error_lgbm}) y pone de manifiesto una limitación estructural de los modelos de árboles: la subestimación sistemática de los picos de consumo extremo, que no pueden extrapolar por encima del rango visto en entrenamiento y que se acentúa al alargar el horizonte. En el horizonte day-ahead (24 horas), el más relevante operativamente, esa
subestimación es mínima. El análisis de importancia de variables, común a ambos modelos de árboles, se presenta en la sección~\ref{sec:shap}.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{CalibXGBoost.png}
    \caption{Calibración rea vs predicho}
    \label{fig:placeholder}
\end{figure}  

\subsubsection{Importancia de variables (SHAP)}

La explicabilidad del modelo se analiza mediante valores SHAP~\cite{lundberg2017} sobre el conjunto de validación. Los resultados confirman la estructura identificada en el EDA. La autorregresión domina la predicción, los tres predictores de mayor contribución son el rezago de una semana (\texttt{lag\_28}), el de 6 horas (\texttt{lag\_1}) y el de 24 horas (\texttt{lag\_4}), que concentran con diferencia la mayor parte del impacto. El gráfico de dependencia del rezago semanal muestra una
relación monótona creciente , un mayor consumo siete días atrás eleva la predicción,  que satura en los valores extremos, coherente con el carácter regular del ciclo de demanda.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{SHAP-Depend.png}
    \caption{SHAP XGBoost Dependences LAG 28}
    \label{fig:placeholder}
\end{figure}

A continuación se sitúa el calendario (hora del día y día de la semana). Entre las variables exógenas, la de mayor peso es la irradiancia solar (\texttt{irradiancia\_mean}), que supera incluso a la identidad del barrio (\texttt{\_level\_skforecast}); las variables de temperatura (CDD, HDD y \texttt{temp\_mean}), en cambio, quedan relegadas a un papel secundario. Esta asimetría sugiere que, a corto plazo y con consumo reciente disponible, la dinámica térmica queda mayoritariamente absorbida por la propia autorregresión, en línea con la evidencia de que el efecto de la temperatura sobre la demanda eléctrica es significativo pero queda parcialmente capturado por la estructura rezagada de la serie~\cite{pardo2002}. 
...
Esto no contradice el papel causal de la temperatura documentado en la literatura, sino que delimita su valor predictivo incremental en horizontes cortos. En el extremo inferior del ranking, \texttt{hora\_x\_finde}
y el perfil sectorial apenas contribuyen; junto con \texttt{cdd\_roll\_3d}, se identifican como candidatas a poda, confirmada formalmente mediante RFECV (sección~\ref{sec:rfecv}).

\subsubsection{Selección de predictores (RFECV)}

El análisis SHAP señala variables de contribución marginal, pero no constituye por sí mismo un criterio formal de eliminación. Como complemento sistemático se aplica una eliminación recursiva de predictores con validación cruzada (RFECV) sobre el bloque de
variables exógenas del modelo global, conservando los seis rezagos y los dos estadísticos de ventana móvil, siguiendo el procedimiento de selección de predictores habitual en el ecosistema skforecast~\cite{skforecast}. El selector , un LightGBM ligero (200 árboles, profundidad 5) con validación cruzada de tres particiones, parte del
conjunto completo de 22 exógenas, estima la importancia de cada una y elimina iterativamente la menos relevante, reevaluando el rendimiento hasta que retirar más variables deja de aportar mejora. Para acotar el coste se emplea un submuestreo del 50\,\% de los datos de entrenamiento (147.060 de 294.120 registros), según la recomendación de que la selección de predictores no requiere ni todo el dato ni el modelo más complejo.

El procedimiento retiene 19 de las 22 exógenas y descarta tres de aportación marginal: \texttt{es\_finde}, \texttt{lst\_nublado} y \texttt{precipitacion\_llueve}. Es relevante que estas no coincidan exactamente con las que el SHAP destacaba como débiles: ambos métodos convergen en que alrededor de tres exógenas son redundantes, pero el criterio formal de RFECV y la importancia SHAP no seleccionan el mismo trío, lo que refuerza la
conveniencia de no apoyar la poda en una única métrica. Al re-evaluar el modelo final sobre el conjunto de test con el subconjunto reducido, el $R^2$ mediano se mantiene sin variación (0,839, idéntico al del modelo completo). Esto confirma que esas tres variables pueden eliminarse sin pérdida de precisión, simplificando el modelo y
reduciendo su riesgo de sobreajuste de cara al despliegue.

\subsection{Deep Learning: LSTM / GRU}

La última familia evaluada son las redes neuronales recurrentes (RNN) LSTM y GRU, cuyos fundamentos se describieron en la sección~II.D. El objetivo es comprobar si su capacidad para aprender dependencias temporales no lineales supera a los modelos de árboles en este problema. La implementación se apoya en skforecast (\texttt{ForecasterRnn}) sobre Keras con backend TensorFlow, con semilla fija (12) en cada entrenamiento para garantizar la reproducibilidad, y se evalúa bajo el mismo backtesting con ventana deslizante a 72 horas que el resto de familias.

\subsubsection{Arquitectura y secuencias de entrada}

Se probaron dos estrategias. Un primer enfoque global multi-serie (una única red que predice las 30 series a la vez, parecido  al modelo global de árboles) que rindió muy por debajo del resto, con un R\textsuperscript{2} mediano de 0,460 en test. Una sola red debe repartir su capacidad entre barrios de magnitud y forma muy distintas (el consumo varía hasta ocho veces entre ellos), lo que coincide con el \textit{benchmark} de skforecast, donde la configuración multi-serie es la menos precisa~\cite{skforecast}. Por ello se adoptó un modelo independiente por código postal, que además permite incorporar el clima propio de cada barrio. Cada red recibe secuencias de los 28 bloques previos (una semana) a través de una capa recurrente de 64 unidades; las series y las exógenas se escalan internamente con \texttt{MinMaxScaler} (transformación reversible) y el entrenamiento emplea \textit{early stopping} sobre la validación (paciencia de seis épocas, restaurando los mejores pesos).

\subsubsection{Ajuste de hiperparámetros}

En lugar de un análisis SHAP, inviable sobre redes recurrentes de secuencia, la selección de variables se realiza midiendo el efecto de retirar bloques de exógenas. Reducir el conjunto de 21 a 11 variables (calendario más el clima esencial, incluidos los grados-día \texttt{HDD} y \texttt{CDD}) fue la selección definitiva. La mediana del subconjunto de exploración asciende de 0,656 a 0,813, mientras que conjuntos más reducidos o el modelo sin exógenas empeoran el resultado.

Por el contrario, las combinaciones de arquitectura (mayor número de unidades, \textit{dropout} o tasas de aprendizaje más lentas) no mejoran la configuración base, y el control de la curva de entrenamiento confirma que las redes no se sobreajustan ya que la diferencia entre entrenamiento y validación es pequeña y refleja el cambio de régimen de 2025 documentado en la auditoría (sección~III.F), no memorización. 

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{OverfitLSTM.png}
    \caption{OverFitting en LSTM}
    \label{fig:placeholder}
\end{figure}

Depuradas las variables, la GRU y el LSTM quedan prácticamente empatados; sobre los 30 códigos la GRU resulta marginalmente superior (R\textsuperscript{2} mediano de 0,800 frente a 0,786 del LSTM, ganando en 18 de los 30). 

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{RNNComparación.png}
    \caption{Comparación R^2 LSTM vs GRU}
    \label{fig:placeholder}
\end{figure}

El hallazgo metodológico es que, en este caso, la combinación de variables  mueve el resultado es la selección de variables y no la arquitectura recurrente concreta. Ambas redes se sitúan al nivel de los modelos estadísticos clásicos y por debajo del gradient boosting, a diferencia del equilibrio favorable al deep learning que reporta Gassar (2024) en un contexto de mayor granularidad y series con características homogéneas~\cite{gassar2024}; la diferencia se puede atribuir a una menor cantidad de datos  por serie y al cambio de comportamiento del periodo de test.

\section{Resultados}

Todos los modelos compartieron el mismo split temporal, el mismo conjunto de 30 códigos postales con series íntegras (sección~III.F) y el mismo periodo de test reservado, lo que garantiza que las diferencias observadas son atribuibles al modelo y no al protocolo de evaluación. 

Siguiendo el criterio adoptado en el análisis exploratorio, la mediana entre códigos postales se emplea como estadístico de referencia por ser más representativa del barrio típico y robusta frente a los pocos códigos de consumo casi nulo que distorsionan la media; la media se reporta en paralelo para la comparación agregada. Cabe mencionar que el conjunto de test abarca únicamente dos meses (octubre y noviembre de 2025), una ventana corta y, además, de volatilidad superior a la histórica (sección~III.F); las cifras de este capítulo deben leerse, por tanto, como una estimación del rendimiento en el periodo más reciente y exigente disponible, no como un promedio anual.
...
De esta comparativa se extraen cinco lecturas. 

1. Cada escalón de complejidad se justifica salvo uno: el SARIMA supera con claridad al \textit{naive} y los modelos de árboles superan al SARIMA, pero el SARIMAX no mejora al SARIMA, lo que confirma que el valor predictivo incremental de las variables exógenas en horizontes cortos es marginal, ya que la autorregresión y la estacionalidad capturan la dinámica de la serie. 

2. Los modelos globales de árboles, entrenados de forma conjunta sobre los 30 códigos, superan a los modelos estadísticos ajustados serie a serie, en línea con la ventaja de los modelos globales al estimar los patrones compartidos con la información agregada de todas las series~\cite{montero2021}; la superioridad del XGBoost sobre el SARIMA es estadísticamente significativa, mejora el R\textsuperscript{2} en 26 de los 30 códigos postales, con un p-valor inferior a 0,001 en un test de Wilcoxon pareado sobre el R\textsuperscript{2} por código (estadístico $W=34$). 

3. Entre los dos modelos de árboles, el XGBoost queda ligeramente por encima del LightGBM (0,846 frente a 0,837 de R\textsuperscript{2} mediano); la diferencia es pequeña (0,009 puntos) aunque el XGBoost gana en 22 de los 30 códigos ($p=0{,}03$ en el mismo test de Wilcoxon), de modo que ambos modelos de gradient boosting lideran de forma prácticamente equivalente y el salto de calidad real sigue estando entre los modelos clásicos y los de árboles. 

4. Las redes recurrentes no alcanzan a los árboles: la GRU (0,800) y el LSTM (0,786) se sitúan al nivel de los modelos estadísticos clásicos (SARIMA 0,808, SARIMAX 0,788) y claramente por debajo del gradient boosting; el contraste de Wilcoxon entre la mejor red (GRU) y el mejor árbol (XGBoost) confirma la ventaja de este último ($p=0{,}003$, la GRU solo supera al XGBoost en 7 de los 30 códigos). Este resultado se analiza en detalle en la sección~IV.C. 

5. La quinta es que la mejora de los árboles se concentra especialmente en los códigos postales de consumo más volátil, precisamente donde el \textit{naive} y el SARIMA acumulan los mayores errores relativos.
...
Tercera, sobre la calidad del dato, la auditoría identificó una diferencia de la fuente que reasigna consumo entre códigos postales en 2025, lo que obligó a excluir doce códigos de la evaluación para garantizar que las métricas reflejan la capacidad de los modelos y no la distorsión de los datos. Este hallazgo metodológico es, en sí mismo, una aportación del trabajo.
