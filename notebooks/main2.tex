...
Sobre el conjunto de test (octubre--noviembre de 2025, horizonte de 72 horas), el SARIMA alcanza un R\textsuperscript{2} mediano de 0,808 (media 0,758) sobre los 30 códigos postales, con un MAE medio de 8.746 MWh y un MAPE medio del 9,1\,\%, superando con holgura al \textit{naive} estacional (R\textsuperscript{2} mediano 0,722, media 0,618). El SARIMAX, en cambio, no mejora al SARIMA, su R\textsuperscript{2} mediano es de 0,788 (media 0,755) con un MAPE medio del 9,3\,\% (en el código 08031 el ajuste del SARIMAX no convergió y queda excluido de su promedio). El SARIMAX no mejora al SARIMA, e incluso lo empeora de forma marginal, por la naturaleza de las exógenas observada en el EDA. Su relación con el consumo es no lineal y, en varios casos, débil: la temperatura describe una U (el consumo crece tanto con el frío como con el calor), lo que se tradujo en correlaciones de Spearman bajas pese a ser significativas, y el viento o la precipitación apenas se relacionan con la demanda. Aunque los grados-día (\texttt{HDD}, \texttt{CDD}) linealizan el efecto térmico, este queda en gran medida absorbido por la diferenciación estacional del SARIMA, con la que es colineal, mientras que la parte genuinamente no lineal un modelo lineal no la puede explotar~\cite{pardo2002}. Así, el bloque exógeno añade varianza de estimación sin señal nueva apreciable, y es esa no linealidad la que en cambio sí aprovechan los modelos de árboles y las redes recurrentes (capítulo~V).
